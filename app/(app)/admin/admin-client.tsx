...
import { RefreshCw, Users, Calendar, Activity, Award, ChevronDown, ChevronUp, Check, X, AlertTriangle, Loader2, Trash2, BarChart2, Trophy, UserMinus } from 'lucide-react'
...
function TabPorras({ porras: initialPorras, onToast }: { porras: PorraRow[]; onToast: (msg: string, type: 'ok' | 'err') => void }) {
  const router = useRouter()
  const [porras, setPorras] = useState<PorraRow[]>(initialPorras)
  const [expandedId, setExpandedId] = useState<string | null>(null)
  const [confirmId, setConfirmId] = useState<string | null>(null)
  const [deletingId, setDeletingId] = useState<string | null>(null)
  const [confirmMember, setConfirmMember] = useState<{ porraId: string; userId: string } | null>(null)
  ...
  const handleRemoveMember = async (porraId: string, userId: string, displayName: string) => {
    setRemovingMember(userId)
    try {
      const res = await fetch(`/api/admin/porras/${porraId}/members/${userId}`, { method: 'DELETE' })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? 'Error desconocido')
      onToast(`${displayName} expulsado de la porra`, 'ok')
      setConfirmMember(null)
      // Update local state without a full refresh
      setPorras(prev => prev.map(p => {
        if (p.id !== porraId) return p
        return {
          ...p,
          member_count: p.member_count - 1,
          members: p.members.filter(m => m.id !== userId),
        }
      }))
    } catch (e: any) {
      onToast(e.message, 'err')
                    ...
                    <td colSpan={4} className="px-6 py-3 bg-[#0f1118]">
                      {p.members.length === 0 ? (
                        ...
                        <div className="flex flex-wrap gap-2">
                          {p.members.map(m => (
                            <div key={m.id} className="flex items-center gap-2 bg-[#13151c] border border-[#1f2333] rounded-lg px-3 py-2">
                              ...
                              {confirmMember?.porraId === p.id && confirmMember?.userId === m.id ? (
                                <div className="flex items-center gap-1.5 ml-1">
                                  <span className="text-zinc-400 text-[11px]">¿Expulsar?</span>
                                  <button
                                    onClick={() => handleRemoveMember(p.id, m.id, m.display_name)}
                                    disabled={removingMember === m.id}
                                    className="flex items-center gap-0.5 px-2 py-0.5 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white text-[11px] font-medium rounded transition-colors cursor-pointer"
                                  >
                                    {removingMember === m.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
                                    Sí
                                  </button>
                                  <button
                                    onClick={() => setConfirmMember(null)}
                                    className="px-2 py-0.5 text-zinc-400 hover:text-white text-[11px] border border-[#2a2f42] rounded transition-colors cursor-pointer"
                                  >
                                    No
                                  </button>
                                </div>
                                ...
                                <button
                                  onClick={() => setConfirmMember({ porraId: p.id, userId: m.id })}
                                  className="ml-1 p-1 text-zinc-600 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors cursor-pointer flex-shrink-0"
                                  title={`Expulsar a ${m.display_name}`}
                                >
